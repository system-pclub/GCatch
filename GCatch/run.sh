...
echo "$GCATCH -path=$GOPATH/src/google.golang.org/grpc -include=google.golang.org/grpc -checker=BMOC -r -compile-error"
$GCATCH -path="$GOPATH"/src/google.golang.org/grpc -include=google.golang.org/grpc -checker=BMOC -r -compile-error -skip=14
