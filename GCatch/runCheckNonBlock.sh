...
echo "GO111MODULE=off $GCATCH -path="$GOPATH"/src/doubleClose -include=doubleClose -checker=BMOC:chSafety -r"
GO111MODULE=off $GCATCH -path="$GOPATH"/src/doubleClose -include=doubleClose -checker=BMOC:chSafety -r
echo ""
echo "-----Step 2.2: running GCatch on testdata/toyprogram/src/sendAfterClose"
echo "GO111MODULE=off $GCATCH -path="$GOPATH"/src/sendAfterClose -include=sendAfterClose -checker=BMOC:chSafety -r"
GO111MODULE=off $GCATCH -path="$GOPATH"/src/sendAfterClose -include=sendAfterClose -checker=BMOC:chSafety -r
