...
func (a *analysis) genRootCalls() *cgnode {
	...
	///MYCODE
	// The following anotated code need an entry point. However, we can just scan all functions
...
func (a *analysis) genFunc(cgn *cgnode) {
	...
	///MYCODE
	// If this function is a non-synthetic non-anonymous method with no caller (How do we know? We use Known_callgraph)
	if Known_callgraph != nil && Recv_to_methods_map != nil { // This is the second run of pointer analysis. We know the callgraph

		// Make sure this is a method, and has no caller
		if fn.Signature.Recv() == nil || fn.Synthetic != "" || strings.Contains(fn.String(),"$") {
			goto Normal
		}
		method := Known_callgraph.Nodes[fn]
		...
		// Now record all other non-synthetic methods that are exported functions (i.e., starting with an Uppercase letter)
		other_exported_methods := []*callgraph.Node{}
		all_methods := Recv_to_methods_map[recv.Type().String()]
		...
		for _, other_method := range all_methods {
			...
			is_exported := false // In Go, a function whose name starts with an Uppercase is exported
			for _,r := range other_method.Func.Name() {
				if unicode.IsUpper(r) {
					is_exported = true
				} else {
					is_exported = false
			...
			other_exported_methods = append(other_exported_methods,other_method)
		}

		// As if fn is called by all functions in other_exported_methods
		for _, other_method := range other_exported_methods {
			a.genFakeCall(other_method.Func,fn)
		}
	}
	...
	for _,bb := range caller.Blocks {
		for _,inst := range bb.Instrs {
			...
			for _,operand := range inst.Operands(nil) {
				if (*operand).Type().String() == caller_param0.Type().String() && (*operand) != caller_param0 {
					param0_arg = *operand
					break outer
				}
			}
